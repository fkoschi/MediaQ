<?php
	...
	// Fetch Data from Wikipedia through API
	if(!empty($_GET['input']))
	{
		$input = $_GET['input'];

		$mediawiki_url = "http://de.wikipedia.org/w/api.php?action=query&titles=".urlencode($input)."&format=json&prop=revisions&rvprop=content";

		$mediawiki_json = file_get_contents($mediawiki_url);
		$mediawiki_array = json_decode($mediawiki_json,true);
		
		$array_keys = array_keys($mediawiki_array['query']['pages'] ); 
		$arrayId = $array_keys[0];

		// print_r("<pre>");
		// echo json_encode($mediawiki_array['query']['pages'][$arrayId]['revisions'][0]['*']);
		// print_r("</pre>");

		$wikitext = trim(stripslashes($mediawiki_array['query']['pages'][$arrayId]['revisions'][0]['*']));
	...
	// 	ob_start();
	// 	require_once "../frameworks/ffnn-php-wikitexttohtml/wikitexttohtml.php";
	// 	ob_end_clean();
?>